...
export default class GameShare extends GameComponent {
  static styles = css`
    ...
    button {
      ...
      border: 2px solid color-mix(in srgb, currentColor 50%, transparent); border-radius: 8px;
      background: transparent; color: var(--game-text, #fff);
      cursor: pointer; transition: transform 0.15s ease, border-color 0.15s ease;
      font-family: inherit;
    }
    button:hover { border-color: currentColor; transform: scale(1.04); }
    button.copied { border-color: var(--game-share-copied, #6ee7b7); color: var(--game-share-copied, #6ee7b7); }
    textarea {
      display: none; width: 100%; max-width: 500px; padding: 10px 14px;
      font-size: 14px; font-family: inherit; color: var(--game-text, #eee);
      background: color-mix(in srgb, currentColor 10%, transparent);
      border: 1px solid color-mix(in srgb, currentColor 25%, transparent);
      border-radius: 6px; text-align: center; user-select: all; -webkit-user-select: all;
      resize: none; field-sizing: content;
    }
    textarea:focus { outline: none; border-color: color-mix(in srgb, currentColor 50%, transparent); }
  `;
