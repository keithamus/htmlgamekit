...
export default class GameTileInput extends GameComponent {
  ...
  static styles = css`
    ...
    @keyframes tile-blink {
      50% { background: var(--game-tile-border, rgba(255, 255, 255, 0.3)); }
    }
  ...
  // value proxies the hidden <input>, not the attribute directly.
  ...
  connectedCallback() {
    ...
    this.#input.addEventListener("input", () => {
      ...
      this.#render();
      this.dispatchEvent(
        new GameTileInputEvent(this.#input.value, this.#input.value.length - 1),
      );
    }, { signal: this.signal });
  ...
  attributeChanged(name) {
    ...
    if (name === "value" && this.#input) {
      this.#input.value = this.getAttribute("value") || "";  // read raw attr; reflected .value proxies the hidden input
      this.#render();
    }
  }
