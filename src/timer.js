...
export default class GameTimer extends GameComponent {
  ...
  static styles = css`
    ...
    :host(:state(warn)) .bar {
      background: var(--game-timer-warn);
    }
    :host(:state(danger)) .bar {
      background: var(--game-timer-danger);
    }
    .sr-only {
      position: absolute;
      width: 1px;
  ...
  static template =
    '<div class="bar"></div><span class="sr-only" role="timer" aria-live="assertive"></span>';
