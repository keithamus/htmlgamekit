...
export default class GameSequencer extends GameComponent {
  ...
  #startHum() {
    const ctx = getAudioCtx();
    const timer = this.shell?.querySelector("game-timer");
    const dur = timer ? timer.duration : 10;
  ...
  #scheduleNote() {
    ...
    if (semitone !== null && semitone !== undefined) {
      ...
      const synth = SYNTHS[this.type] || SYNTHS.marimba;
      synth(ctx, freq, this.#nextTime - ctx.currentTime, gain, stepDur * 1.6);
    }
