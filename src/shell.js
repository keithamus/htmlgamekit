...
async function initGroup(scores, storageKey = "game-group") {
  ...
  if (newGroupName && scores.createGroup) {
    ...
    if (group?.id) {
      ...
      history.replaceState(null, "", clean ? `${location.pathname}?${clean}` : location.pathname);
  ...
  if (groupId) {
    ...
    try {
      localStorage.setItem(storageKey, JSON.stringify({ id: groupId, name }));
    } catch {}
...
export default class GameShell extends HTMLElement {
  // Attr IDL names use `prop:` overrides where the natural camelCase would
  // collide with a Signal.State field (gameId, rounds, scoreOrder, etc.).
  static attrs = {
    "game-id":       { type: "string", prop: "gameIdAttr" },
    "rounds":        { type: "long", default: 0, prop: "roundsAttr" },
    "score-order":   { type: "enum", values: ["asc", "desc"], default: "asc", prop: "scoreOrderAttr" },
    "between-delay": { type: "string", default: "500", prop: "betweenDelayAttr" },
  ...
  roundScore = new Signal.Computed(() => { const rs = this.roundScores.get(); return rs.length ? rs.at(-1) : 0; });
  bestRoundScore = new Signal.Computed(() => { const rs = this.roundScores.get(); return rs.length ? rs.reduce((a, b) => Math.max(a, b), -Infinity) : 0; });
  worstRoundScore = new Signal.Computed(() => { const rs = this.roundScores.get(); return rs.length ? rs.reduce((a, b) => Math.min(a, b), Infinity) : 0; });
  ...
  #slot = (() => { const s = document.createElement("slot"); this.#shadow.appendChild(s); return s; })();
  ...
  attributeChanged(name) {
    ...
    if (name === "between-delay") this.betweenDelay.set(this.#parseBetweenDelay());
    if (name === "game-id") {
  ...
  get gameTitle() {
    return this.querySelector("[when-some-scene~=intro] h1, [when-eq-scene=init] h1, [when-some-scene~=init] h1")
  ...
  connectedCallback() {
    ...
    this.addEventListener("game-trophy-unlock", (e) => {
      e.stopPropagation();
    ...
    this.#effect(() => {
      ...
      prevScene = scene;
      // Run side-effects outside reactive tracking: this effect must depend
      // only on `scene`. Writing signals inside (e.g. encodedResult in
      // #onResult) would over-subscribe and stall the watcher.
      Signal.subtle.untrack(() => {
        this.#syncCustomStates(scene);
        if (scene === "result") this.#onResult();
        this.dispatchEvent(new GameLifecycleEvent(scene, this.#stateSnapshot()));
      });
    });
    ...
    this.#effect(() => {
      ...
      // Autofocus the first [autofocus] element in a newly visible child
      requestAnimationFrame(() => {
        for (const child of visible) {
          const af = child.hasAttribute("when-some-scene") || child.hasAttribute("when-eq-scene")
            ? child.querySelector("[autofocus]")
            : null;
          if (af) { af.focus(); break; }
        }
      });
    });
    ...
    this.#effect(() => {
      ...
      if (scene === "playing" || scene === "between") {
        ...
        try { sessionStorage.removeItem(`${sk}-session`); } catch {}
      }
    });

    this.addEventListener("game-round-pass", (e) => {
      e.stopPropagation();
    ...
    this.addEventListener("game-round-fail", (e) => {
      e.stopPropagation();
      if (this.scene.get() !== "playing") return;
    ...
    this.addEventListener("game-timer-expired", (e) => {
      e.stopPropagation();
      if (this.scene.get() !== "playing") return;
      this.#roundFail("Time's up!", false);
    }, { signal });

    this.addEventListener("game-stat-update", (e) => {
      e.stopPropagation();
      this.stats.set({ ...this.stats.get(), [e.key]: e.value });
    }, { signal });

    this.addEventListener("game-start-request", (e) => {
      e.stopPropagation();
      this.start();
    }, { signal });

    this.addEventListener("game-restart-request", (e) => {
      e.stopPropagation();
      this.start();
    }, { signal });

    this.addEventListener("game-practice-start", (e) => {
      e.stopPropagation();
      this.scene.set("practice");
    }, { signal });

    this.addEventListener("game-complete", (e) => {
      e.stopPropagation();
      if (e.score != null) this.score.set(e.score);
      this.scene.set("result");
    }, { signal });

    this.addEventListener("game-pause-request", (e) => {
      e.stopPropagation();
      this.pause();
    }, { signal });

    this.addEventListener("game-resume-request", (e) => {
      e.stopPropagation();
      this.resume();
    }, { signal });

    this.addEventListener("game-next-round", (e) => {
      e.stopPropagation();
  ...
  async #init() {
    ...
    if (this.getAttribute("group") !== null) {
      const key = this.getAttribute("group") || `${this.storageKey.get()}-group`;
  ...
  #restoreSession() {
    ...
    try {
      const raw = sessionStorage.getItem(`${this.storageKey.get()}-session`);
      if (!raw) return false;
      const saved = JSON.parse(raw);
  ...
  #restoreResult() {
    try {
      const raw = localStorage.getItem(this.storageKey.get());
      if (!raw) return false;
      const saved = JSON.parse(raw);
      if (saved && typeof saved.score !== "undefined") {
        this.scene.set("result");
  ...
  #roundPass(roundScore, feedback) {
    ...
    const total = this.scoreOrder.get() === "asc"
  ...
  #createProgression() {
    const CTORS = { fixed: FixedProgression, staircase: StaircaseProgression, tier: TierProgression };
    ...
    for (const attr of this.attributes) {
      ...
      try { config[key] = JSON.parse(attr.value); } catch { config[key] = attr.value; }
    }
  ...
  #loadTrophies() {
    try {
      const raw = localStorage.getItem(this.#trophyStorageKey);
      if (raw) {
  ...
  #saveTrophies() {
    try {
      localStorage.setItem(this.#trophyStorageKey, JSON.stringify([...this.#trophyUnlocked]));
    } catch {}
  }
    ...
    if (this.#progression) {
      ...
      if (result.done) {
        ...
        if (this.#progression.computeThreshold) {
          this.score.set(Math.round(this.#progression.computeThreshold() * this.scoreScale));
        }
        this.scene.set("result");
        return;
      }
