...
export default class GameScoreForm extends GameComponent {
  static styles = css`
    ...
    label {
      display: block; font-size: 14px; opacity: 0.6;
      margin-bottom: 6px; text-transform: uppercase; letter-spacing: 0.06em;
    }
    .row { display: flex; gap: 8px; justify-content: center; align-items: stretch; }
    input {
      width: 140px; padding: 8px 12px; font-size: 16px; font-family: inherit;
      font-weight: 700; text-transform: uppercase; text-align: center;
      letter-spacing: 0.1em; color: var(--game-text, #eee);
      background: color-mix(in srgb, currentColor 10%, transparent);
      border: 1px solid color-mix(in srgb, currentColor 25%, transparent);
      border-radius: 6px;
    }
    input:focus { outline: none; border-color: color-mix(in srgb, currentColor 50%, transparent); }
    button {
    ...
    .submitted { font-size: 14px; opacity: 0.6; margin-top: 8px; }
  `;
  ...
  #updateForm(shell) {
    ...
    if (!scores?.token) {
      ...
      scores?.fetchToken?.().then(() => this.#updateForm(shell));
    } else {
