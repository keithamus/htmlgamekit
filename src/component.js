...
function coerce(raw, spec) {
  ...
  if (raw === null) {
    ...
    if (spec.type === "number?") return spec.default ?? null;
    if (spec.type === "enum") return spec.missing ?? spec.default ?? null;
    return (
      spec.default ?? (spec.type === "long" || spec.type === "number" ? 0 : "")
    );
  }
  ...
  if (spec.type === "number?") {
    const n = parseFloat(raw);
    return Number.isFinite(n) ? n : (spec.default ?? null);
  }
...
export function initAttrs(Ctor) {
  ...
  const existingList = existing?.get
    ? existing.get.call(Ctor)
    : (existing?.value ?? []);
...
export default class GameComponent extends HTMLElement {
  ...
  connectedCallback() {
    ...
    if (typeof this.triggerCallback === "function") {
      const hasTimeout =
        typeof this.timeoutCallback === "function" ||
