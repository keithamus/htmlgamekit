...
export default class GameScoreHistogram extends GameComponent {
  ...
  static styles = css`
    :host {
      display: block; margin-top: 16px; width: 100%; max-width: 320px;
      color: var(--game-text, #eee);
    }
    .label {
      display: flex; justify-content: space-between; font-size: 11px;
      text-transform: uppercase; letter-spacing: 0.08em; opacity: 0.5; padding: 0 2px 6px;
    }
    .bars {
      display: flex; align-items: flex-end; gap: 2px; height: 80px;
    }
    .bar {
      flex: 1; background: color-mix(in srgb, currentColor 15%, transparent);
      border-radius: 2px 2px 0 0; height: var(--h); position: relative;
      transition: background 0.2s;
    }
    .bar.you { background: var(--game-accent, #3b82f6); }
    .you-label {
      position: absolute; top: -18px; left: 50%; transform: translateX(-50%);
      font-size: 10px; font-weight: 700; color: var(--game-accent, #3b82f6);
      white-space: nowrap; text-shadow: none;
    }
    .axis {
      display: flex; justify-content: space-between; font-size: 10px;
      opacity: 0.35; padding: 4px 2px 0;
    }
  `;
  ...
  async resultCallback(shell) {
    ...
    this.#render(data, shell.score.get());
  }

  #render(data, playerScore) {
    ...
    if (!maxCount) { barsEl.innerHTML = ""; return; }
    ...
    const better = playerBucket < 0 ? 0 : buckets
      .slice(0, playerBucket).reduce((sum, b) => sum + b.count, 0);
    const pct = Math.max(1, Math.ceil(((better + 1) / total) * 100));
