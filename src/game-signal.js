...
export default class GameSignal extends GameComponent {
  ...
  effectCallback({ round, rounds, score, formatScoreSignal }) {
    ...
    if (!key) { this.textContent = ""; return; }

    // "round" shows "N/M" when total rounds are configured.
    if (key === "round") {
      const r = round.get();
    ...
    // "score" uses formatScoreSignal when set.
    if (key === "score") {
      const s = score.get();
      const fmt = formatScoreSignal.get();
      if (fmt) { this.textContent = fmt(s); return; }
    }

    const val = resolve(key, this.shell);
    if (val == null) { this.textContent = ""; return; }
