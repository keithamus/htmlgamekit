...
export class GameSample extends HTMLElement {
  ...
  play(state) {
    ...
    if (vib !== "off") {
      if (vib === "auto" || vib === "") {
        this.#vibrate(type === "noise"
          ? Math.round((duration ?? 0.02) * 1000)
          : vibrateFromNotes(notes, duration));
      } else {
        const parts = vib.split(",").map(Number).filter((n) => !Number.isNaN(n));
...
export default class GameAudio extends GameComponent {
  ...
  triggerCallback(triggerName, event) {
    ...
    for (const el of this.querySelectorAll(`game-sample[trigger="${triggerName}"]`)) {
      if (!matchesConditions(el, this.shell)) continue;
      const val = el.value;
      if (val !== null && event) {
        const eventVal = event.seconds ?? event.value ?? event.detail;
        if (String(eventVal) !== val) continue;
      }
      this.#playSample(el, null);
    }
  }
