export default class StaircaseProgression {
  ...
  constructor({
    ...
    if (levels) {
      this.#levels = levels;
    } else if (levelsStart != null && levelsEnd != null && levelsSteps != null) {
      this.#levels = StaircaseProgression.computeLevels(
        Number(levelsStart), Number(levelsEnd), Number(levelsSteps), levelsScale,
      );
    } else {
      this.#levels = [1];
    }
  ...
  computeThreshold() {
    // Classic psychophysical staircase: average the last up-to-6 reversal
    // values, requiring at least 4 to avoid noise from early reversals.
