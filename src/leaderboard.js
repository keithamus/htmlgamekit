...
export default class GameLeaderboard extends GameComponent {
  ...
  static styles = css`
    ...
    th {
      ...
      opacity: 0.5; padding: 4px 8px; text-align: left;
    }
    th:last-child { text-align: right; }
    td { padding: 4px 8px; border-top: 1px solid color-mix(in srgb, currentColor 8%, transparent); }
    td:first-child { opacity: 0.4; width: 30px; }
    td:last-child { text-align: right; font-weight: 600; }
    .sep td {
      text-align: center; border-top: none; opacity: 0.3;
      padding: 2px 8px; font-size: 12px; letter-spacing: 0.2em;
    }
  `;
  ...
  async resultCallback(shell) {
    ...
    if (worst?.scores) worst = { ...worst, scores: worst.scores.slice(0, worstN) };
  ...
  #render(best, worst, shell) {
    ...
    if (!hasTop && !hasWorst) { table.innerHTML = ""; return; }
    ...
    for (const e of (worst?.scores || []).slice().reverse()) tbody.appendChild(this.#row(e, shell));
