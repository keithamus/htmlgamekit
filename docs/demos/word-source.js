import "./demo-init.js";
import { GameRoundPassEvent, GameRoundFailEvent, gameWordContext } from "../../src/index.js";
...
src.subscribe(gameWordContext, (word) => { target = word; });
...
shell.addEventListener("game-tile-submit", (e) => {
  ...
  for (let i = 0; i < 5; i++) {
    ...
    if (idx !== -1) { states[i] = "close"; remaining[idx] = null; }
  }
  ...
  if (guess === target) {
    ...
    shell.dispatchEvent(new GameRoundPassEvent(6 - row, `Got it in ${row + 1}!`));
    return;
  }
  ...
  if (row >= 6) {
    ...
    shell.dispatchEvent(new GameRoundFailEvent(`It was ${target.toUpperCase()}`));
    return;
  }
