...
```html
<game-shell id="game" game-id="click-counter" rounds="10" score-order="asc"
            between-delay="300"
...
Place a `<game-leaderboard>` in the result overlay to show top scores and a histogram:
...
- **Score histogram** -- a bar chart showing the full distribution, with a "You" marker at the player's score and a "Top N%" label.
...
<game-result-message>
  ...
  <option when-min-score="2001" when-max-score="4000">Solid clicks. Good rhythm.</option>
...
// Encode the score into a URL-safe format
...
// Decode a challenge from the URL
shell.decodeResult = (bytes) => ({
  score: (bytes[0] << 8) | bytes[1],
});
```
...
<div when-some-scene="result" data-overlay>
  ...
  <game-result-stat format="ms" label="Total time"
    animate="800">
  </game-result-stat>
...
```html
<game-shell score-url="https://api.example.com" ...>
```
