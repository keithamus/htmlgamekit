...
```html
<game-shell game-id="reaction-time" score-url="https://scores.example.com">
```
</dd>
...
```html
<game-shell scenes="practice tutorial,review">
```
...
```html
<game-shell game-id="jnd-test" progression="staircase"
            score-scale="100000">
```
</dd>
...
```html
<game-shell id="game" game-id="my-game" session-save>
```
</dd>
...
```js
shell.encodeResult = (state) => GameShell.toBase64Url(new Uint8Array(state.scores));
```
</dd>
...
<game-shell id="game" ...>
  ...
  <game-toast when-some-scene="playing between paused" trigger="pass">Nice!</game-toast>
