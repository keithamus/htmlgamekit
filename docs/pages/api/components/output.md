...
layout: doc
title: "GameSignal"
permalink: /api/components/output/
---

A lightweight element that binds to any shell signal, stat, or difficulty
property and renders its value as text content. No shadow DOM, no styles --
just reactive text. Use it anywhere you need to display a live game value.

{% include "demo-embed.html", demo: "output", title: "Output elements demo", height: "400px" %}
...
<dd>
<code>string</code> -- The value to display. Case-insensitive. Resolved in
order:
...
**`score`** -- Uses `shell.formatScoreSignal` if set, otherwise falls back to
the `format` attribute or raw string.

**`round`** -- Shows `round/rounds` when total rounds are configured, or just
the round number for open-ended games.
...
- **Reactive** -- re-renders via `effectCallback` when the bound signal changes.
  Only the specific signal(s) referenced by `key` are tracked as dependencies.
- **Case-insensitive** -- `key="score"` and `key="Score"` both work.
- **Composable** -- use inside HUDs, between screens, overlays, toasts, or
  custom components.
