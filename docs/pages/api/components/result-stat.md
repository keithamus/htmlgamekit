...
{% include "demo-embed.html", demo: "result-stat", title: "Result stat demo", height: "400px" %}
...
```css
game-result-stat:state(perfect) { color: gold; }
game-result-stat:state(high)::part(value) { color: #00e5b0; }
game-result-stat:state(low)::part(value) { color: #ff3b5c; }
```
...
{% cem_cssprops "game-result-stat" %}

### Usage

```html
<div when-some-scene="result" data-overlay>
  <game-result-stat label="Your Score" animate="800"
    score-low="0" score-mid="10" score-high="15">
  </game-result-stat>
