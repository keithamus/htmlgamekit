...
describe("red-team: component edge cases", () => {
  ...
  describe("trophy edge cases", () => {
    ...
    it("trophy without an id still unlocks cleanly", async () => {
      ...
      // Should not throw; trophyId is "" but unlock() still marks it unlocked
      trophy.unlock();
      assert.isTrue(trophy.unlocked);
    });
  });
  ...
  describe("timer edge cases", () => {
    ...
    it('duration="NaN" — bar width becomes NaN% but no crash', () => {
      ...
      const timer = document.querySelector("game-timer");
      // duration attr "NaN" is not finite, so coerce returns the default (10)
      assert.equal(timer.duration, 10);
      // Start and stop immediately — no crash
      timer.start();
      timer.stop();
    });
    ...
    it("double start() without stop — second start calls stop internally, no leaked rAF", () => {
      ...
      const timer = document.querySelector("game-timer");
      timer.start();
      // Second start should cancel the previous rAF loop via internal stop()
      timer.start();
      let tickCount = 0;
      timer.addEventListener("game-timer-tick", () => tickCount++);
      return new Promise((resolve) => {
        requestAnimationFrame(() => {
          // Only one rAF loop should be active; we'd get at most 1 tick per frame
  ...
  describe("toast edge cases", () => {
    ...
    it("show(undefined) creates a toast element (textContent is empty string)", () => {
      ...
      // DOM textContent setter coerces undefined to "" (not "undefined")
  ...
  describe("tile-input edge cases", () => {
    ...
    it("non-alpha characters are stripped from input", () => {
      ...
      document.body.appendChild(el);
      const input = el.shadowRoot.querySelector("input");
      // Simulate typing non-alpha characters
      input.value = "123!@#";
    ...
    it("input exceeding maxLength is handled by browser truncation", () => {
      ...
      document.body.appendChild(el);
      const input = el.shadowRoot.querySelector("input");
      // Browser's maxLength enforces this, but we can set value directly
      input.value = "ABCDEF";
      input.dispatchEvent(new Event("input"));
      // After regex strip + maxLength, value should be truncated by browser
      // The component doesn't explicitly truncate; maxLength on the input does
      const val = input.value;
      // Value should be alpha only
    ...
    it("mark() with out-of-bounds position does not crash", () => {
      const el = document.createElement("game-tile-input");
      document.body.appendChild(el);
      // mark() just adds to a Map; no bounds checking needed since
      // render only looks up marks that match existing tile indices
      el.mark(999, "z", "good");
      el.mark(-1, "a", "bad");
      // No crash is the assertion — force a render
      el.value = "abc";
    ...
    it("showResult() with mismatched array lengths does not crash", () => {
      ...
      document.body.appendChild(el);
      // Fewer elements than length
      el.showResult(["a", "b"], ["good"]);
      // More elements than length
      el.showResult(["a", "b", "c", "d", "e", "f", "g"], ["good", "bad", "close", "wrong", "good", "good", "bad"]);
  ...
  describe("between edge cases", () => {
    it("game-between with no data-between children does not crash", async () => {
      ...
      shell.start();
      await microtask();
      // Trigger a round pass to go to between scene
      shell.lastFeedback.set("Test");
      shell.scene.set("between");
      await microtask();
      assert.equal(shell.scene.get(), "between", "should reach between scene without crash");
    });

    it("game-between outside of game-shell does not crash", async () => {
      document.body.innerHTML = "<game-between><span data-between='feedback'></span></game-between>";
      await tick();
      const between = document.querySelector("game-between");
      // shell is null, connectedCallback returns early
  ...
  describe("quiz edge cases", () => {
    ...
    it("quiz with all radios already checked unchecks them on round setup", async () => {
      ...
      shell.start();
      await microtask();
      // After round setup, all radios should be unchecked
      const radios = document.querySelectorAll("game-quiz input[type=radio]");
      for (const radio of radios) {
        assert.isFalse(radio.checked, "radio should be unchecked after round setup");
      }
    });
  });
  ...
  describe("preferences edge cases", () => {
    it('preference with unknown type falls back to "toggle"', () => {
      ...
      // Enum type with values: ["toggle", "range"], invalid: "toggle"
    ...
    it("corrupt localStorage data does not crash — loads defaults", async () => {
      ...
      assert.equal(prefs.get("sound"), true, "should use default after corrupt localStorage");
    });
  });
  ...
  describe("component lifecycle", () => {
    it("component outside game-shell — shell is null, connectedCallback returns early", async () => {
      ...
      // Timer should still be creatable and have its shadow DOM
      assert.isNotNull(timer.shadowRoot);
      // Manual start/stop should still work
      timer.start();
      timer.stop();
    });

    it("shell signals remain functional after a sibling component is removed", async () => {
      // Verify that removing a component does not break the shell's signal system.
      document.body.innerHTML = `
      ...
      await microtask();
      assert.equal(shell.scene.get(), "ready", "shell still functional after sibling removal");
    });
  });
});
