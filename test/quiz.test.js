...
describe("game-quiz", () => {
  ...
  describe("question indexing", () => {
    it("all fieldsets start hidden after connecting inside a shell", async () => {
      ...
      for (const fs of fieldsets) {
        assert.isTrue(
          fs.hidden,
          `fieldset "${fs.querySelector("legend")?.textContent}" should be hidden`,
        );
      }
    });
  ...
  describe("round setup", () => {
    it("shows a fieldset when game starts playing", async () => {
      ...
      assert.equal(
        visibleFieldsets.length,
        1,
        "exactly one fieldset should be visible",
      );
    });
    ...
    it("shuffles label order within the fieldset", async () => {
      ...
      for (const fs of quiz.querySelectorAll("fieldset")) {
        originalOrders.push(
          Array.from(fs.querySelectorAll("label")).map((l) =>
            l.textContent.trim(),
          ),
        );
      }
      ...
      for (let i = 0; i < 10; i++) {
        ...
        const origForTier =
          originalOrders[
            Array.from(
              quiz.querySelectorAll(`fieldset[data-tier="${tierIdx}"]`),
            ).indexOf(visible)
          ];
  ...
  describe("correct answer", () => {
    ...
    it("disables all inputs after selecting correct answer", async () => {
      ...
      for (const input of visible.querySelectorAll("input[type=radio]")) {
        assert.isTrue(
          input.disabled,
          "all inputs should be disabled after answer",
        );
      }
    });
  });
  ...
  describe("multi-round flow", () => {
    it("hides previous fieldset and shows a new one each round", async () => {
      ...
      assert.equal(
        visibleFieldsets.length,
        1,
        "exactly one fieldset visible in round 2",
      );
    });
    ...
    it("deactivates quiz when scene leaves playing/between", async () => {
      ...
      if (visible) {
        ...
        assert.equal(
          events.length,
          0,
          "should not dispatch events when inactive",
        );
      }
    });
  });
