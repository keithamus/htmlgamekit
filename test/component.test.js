...
describe("initAttrs / attribute reflection", () => {
  ...
  describe("long attribute", () => {
    it("returns the default value when the attribute is absent", () => {
      // countdown has type: "long", default: 0
  ...
  describe("boolean attribute", () => {
    it("returns false when the attribute is absent", () => {
      // Use game-shell for a boolean attr: "demo"
      document.body.innerHTML = "";
  ...
  describe("string? (nullable) attribute", () => {
    it("returns null when absent", () => {
      // game-toast has value: { type: "string?" }
      document.body.innerHTML = "";
  ...
  describe("enum attribute", () => {
    it("returns default when attribute is absent", () => {
      // game-toast position: { type: "enum", values: [...], default: "center" }
      document.body.innerHTML = "<game-toast></game-toast>";
    ...
    it("missing overrides default for absent attribute only", () => {
      class TestEl extends HTMLElement {
        static attrs = {
          mode: { type: "enum", values: ["a", "b"], default: "a", missing: "b" },
        };
        static define(tag, reg = customElements) { initAttrs(this); reg.define(tag, this); }
      }
      TestEl.define("test-enum-missing");
      document.body.innerHTML = '<test-enum-missing></test-enum-missing>';
      assert.equal(document.querySelector("test-enum-missing").mode, "b", "absent → missing");
      document.body.innerHTML = '<test-enum-missing mode="x"></test-enum-missing>';
      assert.equal(document.querySelector("test-enum-missing").mode, "a", "invalid → default, not missing");
    });

    it("invalid overrides default for invalid values only", () => {
      class TestEl2 extends HTMLElement {
        static attrs = {
          mode: { type: "enum", values: ["a", "b"], default: "a", invalid: "b" },
        };
        static define(tag, reg = customElements) { initAttrs(this); reg.define(tag, this); }
      }
      TestEl2.define("test-enum-invalid");
      document.body.innerHTML = '<test-enum-invalid></test-enum-invalid>';
      assert.equal(document.querySelector("test-enum-invalid").mode, "a", "absent → default, not invalid");
      document.body.innerHTML = '<test-enum-invalid mode="x"></test-enum-invalid>';
      assert.equal(document.querySelector("test-enum-invalid").mode, "b", "invalid → invalid override");
    });
  });
  ...
  describe("initAttrs idempotency", () => {
    it("is safe to call multiple times (no-op second call)", () => {
      ...
      initAttrs(Ctor);
      initAttrs(Ctor); // second call should not throw or double-define
...
describe("GameComponent lifecycle", () => {
  ...
  describe("shell getter", () => {
    ...
    it("returns the game-shell when inside one", async () => {
      document.body.innerHTML = '<game-shell><game-flash></game-flash></game-shell>';
      const shell = document.querySelector("game-shell");
      const flash = document.querySelector("game-flash");
  ...
  describe("effectCallback(shell)", () => {
    it("is called reactively when shell signals change", async () => {
      document.body.innerHTML = '<game-shell rounds="3" between-delay="0"><game-flash></game-flash></game-shell>';
      await tick();
      const shell = document.querySelector("game-shell");
      const flash = document.querySelector("game-flash");
      // game-flash's effectCallback reads scene and lastRoundPassed.
      shell.start();
      assert.equal(shell.scene.get(), "playing");
      shell.dispatchEvent(new (await import("../src/events.js")).GameRoundPassEvent(1));
      assert.equal(shell.scene.get(), "between");
      // Wait for microtask batch to process signal changes.
  ...
  describe("disconnectedCallback", () => {
    ...
    it("clears the cached shell reference", async () => {
      ...
      flash.remove();
      // After disconnect, shell is cleared; re-connecting outside shell yields null
      document.body.appendChild(flash);
      // Force a fresh lookup (shell cache was cleared on disconnect)
      assert.isNull(flash.shell);
    });
  });
});
