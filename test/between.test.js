...
describe("game-between", () => {
  ...
  it("creates from document.createElement", () => {
    const el = document.createElement("game-between");
    assert.equal(el.nodeName, "GAME-BETWEEN");
  });
  ...
  it("populates [data-between=feedback] with lastFeedback", async () => {
    ...
    shell.start();
    await microtask();
    document.querySelector("#trigger").dispatchEvent(new GameRoundPassEvent(10, "Great job!"));
    await microtask();
  ...
  it("populates [data-between=score] with current score", async () => {
    ...
    shell.start();
    await microtask();
    document.querySelector("#trigger").dispatchEvent(new GameRoundPassEvent(10, "Nice"));
    await microtask();
