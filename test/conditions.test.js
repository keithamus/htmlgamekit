...
describe("matchesConditions", () => {
  ...
  describe("when-min-{stat} / when-max-{stat}", () => {
    ...
    it("fails when stat value < N", () => {
      el.setAttribute("when-min-streak", "3");
      assert.isFalse(matchesConditions(el, makeShell({ stats: { streak: 2 } })));
    });
    ...
    it("passes when stat value <= N", () => {
      ...
      assert.isTrue(matchesConditions(el, makeShell({ stats: { avgTime: 400 } })));
      assert.isFalse(matchesConditions(el, makeShell({ stats: { avgTime: 600 } })));
    });
  });
  ...
  describe("when-eq-{key}", () => {
    ...
    it("matches difficulty property by normalised key", () => {
      ...
      assert.isTrue(matchesConditions(el, makeShell({ difficulty: { tierName: "Hard" } })));
      assert.isFalse(matchesConditions(el, makeShell({ difficulty: { tierName: "Easy" } })));
    });
  });
  ...
  describe("when-some-trophy / when-no-trophy", () => {
    ...
    it("when-no-trophy fails when trophy IS unlocked", () => {
      el.setAttribute("when-no-trophy", "gold-star");
      assert.isFalse(matchesConditions(el, makeShell({ unlockedTrophies: new Set(["gold-star"]) })));
    });
  });
  ...
  describe("when-min-round-score / when-max-round-score", () => {
    it("when-min-round-score passes when last round score >= N", () => {
      ...
      assert.isTrue(matchesConditions(el, makeShell({ roundScores: [5, 8, 10] })));
    });
    ...
    it("when-max-round-score fails when best round score > N", () => {
      el.setAttribute("when-max-round-score", "10");
      assert.isFalse(matchesConditions(el, makeShell({ roundScores: [5, 8, 11] })));
    });
  });
  ...
  describe("when-some-{key} / when-no-{key}", () => {
    it("when-some with no value: passes when value is truthy", () => {
      ...
      assert.isTrue(matchesConditions(el, makeShell({ stats: { streak: 3 } })));
      assert.isFalse(matchesConditions(el, makeShell({ stats: { streak: 0 } })));
    });
    ...
    it("when-no with no value: passes when value is falsy", () => {
      ...
      assert.isFalse(matchesConditions(el, makeShell({ stats: { streak: 3 } })));
    });
    ...
    it("when-some-trophy with multiple ids: passes when any is unlocked", () => {
      ...
      assert.isTrue(matchesConditions(el, makeShell({ unlockedTrophies: new Set(["star"]) })));
      assert.isTrue(matchesConditions(el, makeShell({ unlockedTrophies: new Set(["crown"]) })));
      assert.isFalse(matchesConditions(el, makeShell({ unlockedTrophies: new Set(["other"]) })));
    });

    it("when-no-trophy with multiple ids: passes when none are unlocked", () => {
      el.setAttribute("when-no-trophy", "star crown");
      assert.isTrue(matchesConditions(el, makeShell({ unlockedTrophies: new Set(["other"]) })));
      assert.isFalse(matchesConditions(el, makeShell({ unlockedTrophies: new Set(["star"]) })));
      assert.isFalse(matchesConditions(el, makeShell({ unlockedTrophies: new Set(["star", "crown"]) })));
    });
  });
  ...
  describe("when-all-{key}", () => {
    it("when-all-trophy: passes when all listed trophies are unlocked", () => {
      ...
      assert.isTrue(matchesConditions(el, makeShell({ unlockedTrophies: new Set(["star", "crown"]) })));
      assert.isFalse(matchesConditions(el, makeShell({ unlockedTrophies: new Set(["star"]) })));
      assert.isFalse(matchesConditions(el, makeShell({ unlockedTrophies: new Set([]) })));
    });
  ...
  describe("multiple conditions (AND logic)", () => {
    it("passes when all conditions are met", () => {
      ...
      el.setAttribute("when-eq-scene", "playing");
      assert.isTrue(matchesConditions(el, makeShell({ score: 15, round: 3, scene: "playing" })));
    });
    ...
    it("fails when any condition is not met", () => {
      ...
      el.setAttribute("when-min-round", "2");
      el.setAttribute("when-eq-scene", "playing");
      assert.isFalse(matchesConditions(el, makeShell({ score: 5, round: 3, scene: "playing" })));
    });
  });
});
