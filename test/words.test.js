...
function wordFetch(word) {
  return async () => ({ ok: true, json: async () => ({ word, day: 1, pool: 100 }) });
}
...
function lookupFetch(found) {
  return async (url) => {
    ...
    return { ok: true, json: async () => ({ word: "crane", day: 1, pool: 100 }) };
  };
}
...
describe("game-word-source", () => {
  ...
  it("creates from document.createElement", () => {
    ...
    assert.equal(el.localName, "game-word-source");
  });
  ...
  it("random mode uses ?random param", async () => {
    const urls = [];
    mockFetch(async (url) => { urls.push(url); return { ok: true, json: async () => ({ word: "crane", pool: 100 }) }; });
  ...
  it("daily mode uses ?day param", async () => {
    const urls = [];
    mockFetch(async (url) => { urls.push(url); return { ok: true, json: async () => ({ word: "frost", pool: 400 }) }; });
  ...
  it("includes length param in fetch URL", async () => {
    const urls = [];
    mockFetch(async (url) => { urls.push(url); return { ok: true, json: async () => ({ word: "planet", pool: 200 }) }; });
  ...
  it("includes theme param when set", async () => {
    const urls = [];
    mockFetch(async (url) => { urls.push(url); return { ok: true, json: async () => ({ word: "mouse", pool: 80 }) }; });
  ...
  it("omits theme param when unset", async () => {
    const urls = [];
    mockFetch(async (url) => { urls.push(url); return { ok: true, json: async () => ({ word: "crane", pool: 400 }) }; });
  ...
  it("does not fetch when words-url is missing", async () => {
    let fetched = false;
    mockFetch(async () => { fetched = true; return { ok: true, json: async () => ({}) }; });
  ...
  it("distributes word via gameWordContext", async () => {
    ...
    let received = "";
    src.subscribe(gameWordContext, (w) => { received = w; });
  ...
  it("daily mode reuses same word across rounds without re-fetching", async () => {
    ...
    mockFetch(async () => {
      fetchCount++;
      return { ok: true, json: async () => ({ word: "pearl", day: 1, pool: 400 }) };
    });
    ...
    // Advance to round 2 via a pass event (shell is in "playing" after start+tick)
    shell.dispatchEvent(new GameRoundPassEvent(1));
    await tick(); // between → playing transition
    await tick(); // effect re-runs
  ...
  it("fires PendingTaskEvent wrapping the fetch promise", async () => {
    ...
    shell.addEventListener("pending-task", (e) => { pendingTask = e; }, { once: true });
  ...
  it("with validate: forwards game-tile-submit when word is in lookup", async () => {
    ...
    let received = null;
    shell.addEventListener("game-tile-submit", (e) => { received = e.value; });
